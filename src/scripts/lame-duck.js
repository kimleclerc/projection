// Lame-Duck Index — client-side module
// Loaded by Astro/Vite: ES module, bundled + minified at build time.
// Islands pattern: above-fold sections init immediately; below-fold use IntersectionObserver.

const DATA_URL = '/web_data/us-lame-duck/latest.json';

// ── Shared data cache (one fetch, all islands share it) ─────────────────────
let _req = null;
function getLdiData() {
  if (!_req) {
    _req = fetch(DATA_URL)
      .then(r => { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
      .catch(err => { console.error('[LDI] fetch failed:', err); return null; });
  }
  return _req;
}
...
function duckSVG(size, bobbing = false) {
  ...
  return '<svg class="' + cls + '" width="' + w + '" height="' + h + '" viewBox="0 0 200 190" style="overflow:visible" aria-hidden="true">'
    ...
    // ── Trump toupet ────────────────────────────────────────────────────────
    // Main hair body: sweeps from left side of head over the top to the right
    + '<path d="M 96 74 Q 92 46 120 30 Q 146 20 168 34 Q 184 46 180 62 Q 177 70 167 72 Q 154 60 138 56 Q 120 54 108 62 Z" fill="oklch(0.84 0.16 78)" stroke="oklch(0.68 0.20 64)" stroke-width="0.7" opacity="0.95"/>'
    // Individual wispy strands (darker)
    + '<path d="M 106 66 Q 114 44 148 30" stroke="oklch(0.60 0.22 60)" stroke-width="2.2" fill="none" opacity="0.55" stroke-linecap="round"/>'
    + '<path d="M 112 63 Q 121 42 155 32" stroke="oklch(0.66 0.20 68)" stroke-width="1.5" fill="none" opacity="0.50" stroke-linecap="round"/>'
    + '<path d="M 100 70 Q 106 50 130 38" stroke="oklch(0.58 0.22 58)" stroke-width="2.5" fill="none" opacity="0.40" stroke-linecap="round"/>'
    // Front forelock / pompadour curl
    + '<path d="M 105 62 Q 112 44 128 42 Q 119 34 109 40 Q 101 48 105 62 Z" fill="oklch(0.80 0.18 74)" opacity="0.90"/>'
    // ── End toupet ─────────────────────────────────────────────────────────
    // Head shine & beak
...
function buildGaugeMeter(value) {
  ...
  const segs = [
    { from: 0,  to: 25,  color: 'var(--blue-soft)', label: t('FULL POWER', 'PLEIN POUVOIR') },
    { from: 25, to: 50,  color: 'oklch(0.62 0.12 170)', label: t('WANING', 'DÉCLINANT') },
    { from: 50, to: 75,  color: 'var(--duck-deep)', label: t('QUACKING', 'QUI COASSE') },
    { from: 75, to: 100, color: 'var(--red)', label: t('FULLY LAME', 'CANARD BOITEUX') },
  ];
  ...
  const segPaths = segs.map(seg => {
    const outer = arc(seg.from, seg.to, r), inner = arc(seg.from, seg.to, ri);
    const active = value >= seg.from && value < seg.to;
    const tt = seg.label === t('FULL POWER','PLEIN POUVOIR') ? t('Presidential agenda advancing; approval strong; Congress aligned.','Agenda présidentiel avançant ; approbation forte ; Congrès aligné.')
             : seg.label === t('WANING','DÉCLINANT') ? t('Legislative friction increasing; polling slipping.','Friction législative croissante ; cotes glissantes.')
             : seg.label === t('QUACKING','QUI COASSE') ? t('Clear lame-duck signals: approval declining, generic ballot adverse.','Signaux clairs : approbation en baisse, intention de vote défavorable.')
             : t('Effectively sidelined; attention has moved to midterms.','Effectivement mis de côté ; attention tournée vers les mi-mandats.');
  ...
  const ticks = [0,25,50,75,100].map(t_v => {
    const a = toRad(t_v), [x1,y1] = pt(a, r+6), [x2,y2] = pt(a, r+14), [xt,yt] = pt(a, r+28);
    return '<line x1="'+x1+'" y1="'+y1+'" x2="'+x2+'" y2="'+y2+'" stroke="var(--ink-3)" stroke-width="1"/>'
      + '<text x="'+xt+'" y="'+(yt+3)+'" font-size="10" font-family="var(--mono)" fill="var(--ink-3)" text-anchor="middle">'+t_v+'</text>';
  });
  ...
  return '<div style="position:relative;width:100%;max-width:420px;margin:0 auto">'
    ...
    + '<div class="eyebrow" style="color:var(--ink-3)">'+t('Current reading','Lecture actuelle')+'</div>'
...
function buildBathtubMeter(value) {
  ...
  const tickLines = [0,25,50,75,100].map(tv => {
    const y = 50 + (1-tv/100)*250+20;
  ...
  return '<div style="display:flex;justify-content:center;padding:8px 0">'
    ...
    + '<text x="232" y="'+(lameLine+4)+'" font-size="9" font-family="var(--mono)" fill="var(--red)">'+t('FULLY LAME','CANARD BOITEUX')+'</text>'
    + '</svg>'
...
function buildWaterlineMeter(value) {
  ...
  const bandRects = colorBands.map(([f,tv,c]) => '<rect x="'+(f*6)+'" y="124" width="'+((tv-f)*6)+'" height="6" fill="'+c+'" opacity=".85"/>').join('');
  const scaleTicks = [0,25,50,75,100].map(tv =>
    '<line x1="'+(tv*6)+'" y1="120" x2="'+(tv*6)+'" y2="134" stroke="var(--ink)" stroke-width="1"/>'
    + '<text x="'+(tv*6)+'" y="118" font-size="10" font-family="var(--mono)" fill="var(--ink-3)" text-anchor="middle">'+tv+'</text>'
  ).join('');
  return '<div style="padding:16px 0">'
    ...
    + '<span>'+t('FULL POWER','PLEIN POUVOIR')+'</span><span>'+t('WANING','DÉCLINANT')+'</span><span>'+t('QUACKING','QUI COASSE')+'</span><span>'+t('FULLY LAME','CANARD BOITEUX')+'</span>'
    + '</div></div>'
...
function renderHeroStats(data) {
  const { ldi, components = [], meta = {} } = data;
  const updEl = document.getElementById('hero-updated');
  if (updEl && meta.as_of_date) updEl.textContent = fmtDate(meta.as_of_date);
  ...
  if (deltaEl && ldi.delta_7d != null) {
    ...
    deltaEl.textContent = (d>0?'▲':d<0?'▼':'—') + ' ' + Math.abs(d).toFixed(1) + ' · 7d';
  }

  const apprComp = components.find(c => c.id === 'net_approval');
  const apprEl = document.getElementById('stat-approval');
  if (apprEl && apprComp) apprEl.textContent = apprComp.raw_label || '—';

  const daysEl = document.getElementById('stat-days');
  if (daysEl && meta.days_to_midterms != null) daysEl.textContent = meta.days_to_midterms;

  const qualNote = document.getElementById('meter-quality-note');
  if (qualNote) qualNote.textContent = ldi.data_quality === 'real'
    ? t('✓ All real data', '✓ Données réelles')
    : t('⚠ Mixed (some estimates)', '⚠ Mixte (estimations)');
...
function renderMeter(data) {
  const v = data.ldi.score;
  const gaugeEl = document.getElementById('meter-gauge');
  const tubEl   = document.getElementById('meter-tub');
  const waterEl = document.getElementById('meter-water');
  if (gaugeEl) gaugeEl.innerHTML = buildGaugeMeter(v);
  if (tubEl)   tubEl.innerHTML   = buildBathtubMeter(v);
  if (waterEl) waterEl.innerHTML = buildWaterlineMeter(v);

  // Meter switcher
  document.querySelectorAll('.meter-switch button').forEach(btn => {
    btn.addEventListener('click', () => {
      const target = btn.dataset.meter;
      document.querySelectorAll('.meter-view').forEach(v => v.classList.remove('active'));
      document.querySelectorAll('.meter-switch button').forEach(b => b.classList.remove('active'));
...
function renderComponentGrid(components) {
  ...
  function dragLabel(score) {
    ...
    if (score >= 66)   return { text: t('↑ Strong drag on LDI', "↑ Fort poids sur l'ICD"), color: 'var(--red-soft)' };
    if (score >= 40)   return { text: t('↑ Moderate drag', '↑ Poids modéré'),   color: 'var(--duck-deep)' };
    return { text: t('↓ Supportive', '↓ Favorable'), color: 'oklch(0.55 0.12 150)' };
  }
  ...
  grid.innerHTML = components.map(c => {
    ...
    const name  = isFr ? (c.name_fr || c.name_en || c.id) : (c.name_en || c.id);
    const tooltip = isFr ? (c.name_fr + '|' + (c.tooltip_fr||'')) : (c.name_en + '|' + (c.tooltip_en||''));
    ...
    return '<div class="comp-cell" data-tooltip="'+tooltip+'">'
      + qBadge
      ...
      + '<div style="font-family:var(--mono);font-size:10px;color:var(--ink-3);margin-top:5px;letter-spacing:.06em">'+t('INDEX SCORE','SCORE INDICE')+' '+(score!=null?score.toFixed(0)+'/100':'—')+'</div>'
      + '</div>';
  }).join('');
}
...
function renderChart(history) {
  ...
  const pathAppr = _chartData
    ...
    .map(obj => { const cmd = apprFirst ? (apprFirst=false,'M') : 'L'; return cmd+' '+xS(obj.i).toFixed(1)+' '+yAppr(obj.p.net_approval).toFixed(1); }).join(' ');
  ...
  if (scrub) {
    ...
    updateScrub(n-1, xS, yLDI, yAppr);
    scrub.addEventListener('input', e => updateScrub(+e.target.value, xS, yLDI, yAppr));
  }
}

function updateScrub(idx, xS, yLDI, yAppr) {
  const p = _chartData[idx]; if (!p) return;
  const x = xS(idx);
  const dateEl = document.getElementById('chart-date');
  if (dateEl) dateEl.textContent = fmtDate(p.date);
  const ldiEl = document.getElementById('chart-ldi-val');
  if (ldiEl) ldiEl.textContent = p.ldi!=null ? p.ldi.toFixed(1) : '—';
  const apprEl = document.getElementById('chart-appr-val');
  if (apprEl) apprEl.textContent = p.net_approval!=null ? (p.net_approval>=0?'+':'')+p.net_approval.toFixed(1)+'pp' : '—';
...
function renderPresidentsGrid(presidents, currentLDI) {
  const grid = document.getElementById('history-grid');
  if (!grid) return;
  function sparkPath(vals) {
    return vals.map((v,i) => (i===0?'M':'L')+' '+((i/(vals.length-1))*130)+' '+(40-(v/100)*40)).join(' ');
  }
  const cells = presidents.map(p => {
    const sp = p.sparkline?.length ? sparkPath(p.sparkline) : '';
    ...
    return '<div class="history-cell" data-pres="'+dataStr+'" role="button" tabindex="0" title="'+t('Click for details','Cliquer pour détails')+'">'
      ...
      + (sp ? '<svg viewBox="0 0 130 40" width="100%" height="40" style="display:block" preserveAspectRatio="none">'
          ...
          + '</svg>' : '<div style="height:40px"></div>')
      + '<div class="history-final">'+(p.final_ldi!=null?p.final_ldi:'—')+'</div>'
      + '<div class="history-label">'+t('Term-end LDI','ICD fin de mandat')+'</div>'
      + '</div>';
  });
  const trumpPath = _chartData.length>1 ? _chartData.map((d,i) => (i===0?'M':'L')+' '+((i/(_chartData.length-1))*130)+' '+(40-(d.ldi/100)*40)).join(' ') : '';
  ...
  cells.push('<div class="history-cell" style="background:oklch(0.95 0.05 90/0.4);border-left:3px solid var(--duck-deep)">'
    ...
    + '<div class="history-label">'+t('Current LDI','ICD actuel')+'</div>'
    + '</div>');
  grid.innerHTML = cells.join('');

  // Event delegation for modal (no inline onclick)
  grid.addEventListener('click', e => {
    const cell = e.target.closest('[data-pres]');
    if (cell) openPresModal(cell.getAttribute('data-pres'));
  });
  grid.addEventListener('keydown', e => {
    if (e.key==='Enter'||e.key===' ') {
      const cell = e.target.closest('[data-pres]');
      if (cell) { e.preventDefault(); openPresModal(cell.getAttribute('data-pres')); }
    }
  });
}

function openPresModal(dataStr) {
  const p = JSON.parse(dataStr.replace(/&quot;/g,'"'));
  const dialog  = document.getElementById('pres-dialog');
  const content = document.getElementById('pres-modal-content');
  if (!dialog||!content) return;
  const partyColor = p.party==='D' ? 'var(--blue)' : 'var(--red)';
  const partyName  = p.party==='D' ? t('Democrat','Démocrate') : t('Republican','Républicain');
  const n = p.sparkline.length;
  const W = 560, H = 180;
  const xs = i => 16 + (i/(n-1))*(W-32);
  const ys = v => H - 16 - (v/100)*(H-32);
  const path = p.sparkline.map((v,i) => (i===0?'M':'L')+' '+xs(i).toFixed(1)+' '+ys(v).toFixed(1)).join(' ');
  const area = path+' L '+xs(n-1).toFixed(1)+' '+H+' L '+xs(0).toFixed(1)+' '+H+' Z';
  const gridLines = [0,25,50,75,100].map(v =>
    '<line x1="16" y1="'+ys(v)+'" x2="'+(W-16)+'" y2="'+ys(v)+'" stroke="var(--rule-2)" stroke-dasharray="2 3"/>'
    + '<text x="10" y="'+(ys(v)+4)+'" font-size="9" font-family="var(--mono)" fill="var(--ink-3)" text-anchor="end">'+v+'</text>');
  const dots = p.sparkline.map((v,i) => (i!==0&&i!==n-1)?'' : '<circle cx="'+xs(i).toFixed(1)+'" cy="'+ys(v).toFixed(1)+'" r="4" fill="'+partyColor+'"/>').join('');
  const zone = v => v>=75?t('Fully Lame','Canard boiteux'):v>=50?t('Quacking','Qui coasse'):v>=25?t('Waning','Déclinant'):t('Full Power','Plein pouvoir');
  ...
  content.innerHTML =
    ...
    + gridLines.join('')+'<path d="'+area+'" fill="var(--duck)" opacity=".12"/>'
    + '<path d="'+path+'" stroke="'+partyColor+'" stroke-width="2" fill="none"/>'+dots+'</svg>'
    + '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;padding-top:16px;border-top:1px solid var(--rule-2)">'
    + '<div><div class="eyebrow" style="color:var(--ink-3);font-size:9px;margin-bottom:4px">'+t('Start of term','Début de mandat')+'</div>'
    ...
    + '<div><div class="eyebrow" style="color:var(--ink-3);font-size:9px;margin-bottom:4px">'+t('End of term','Fin de mandat')+'</div>'
    ...
    + '<div><div class="eyebrow" style="color:var(--ink-3);font-size:9px;margin-bottom:4px">'+t('Change','Changement')+'</div>'
    + '<div style="font-family:var(--serif);font-size:28px">'+(diff>=0?'+':'')+diff.toFixed(0)+'</div>'
    + '<div style="font-family:var(--mono);font-size:10px;color:var(--ink-3)">'+t('over term','sur le mandat')+'</div></div>'
    + '</div>'
    + '<p style="font-size:12px;color:var(--ink-3);margin-top:16px;font-family:var(--mono);line-height:1.6">'
    + t('Retroactive LDI computed from real historical data: approval (Gallup archive), generic ballot (ANES estimates), congressional control (historical record), and UMCSENT (FRED).',
       'ICD rétroactif calculé à partir de données historiques réelles : approbation (archive Gallup), intention de vote (estimations ANES), contrôle du Congrès (historique), et UMCSENT (FRED).')
    + '</p>';
  dialog.showModal();
}

function renderMidterms(midterms) {
  ...
  function chamberCard(chamber, demSeats, gopSeats, totalSeats, maj, pDem) {
    ...
    const badgeText  = demFav ? t('Dem favored','Dem favoris') : gopFav ? t('GOP favored','GOP favoris') : t('Toss-up','Bal. égale');
    const over = demSeats > maj ? '(+' + (demSeats-maj) + ')' : '';
    return '<div class="chamber-card">'
      + '<div class="chamber-title-row">'
      + '<h3 class="chamber-title">'+t('U.S. ','')+''+chamber+'</h3>'
      + '<span class="chamber-badge '+badgeClass+'">'+badgeText+'</span>'
      + '</div>'
      + '<div>'
      + '<div class="seat-bar-labels"><span style="color:var(--blue)">Dems '+demSeats+' '+over+'</span><span style="color:var(--red)">GOP '+gopSeats+'</span></div>'
      + '<div class="seat-bar"><div class="b-dem" style="width:'+demPct+'%"></div><div class="b-gop" style="flex:1"></div></div>'
      + '<div class="seat-bar-legend"><span>0</span><span>'+maj+' ← '+t('majority','majorité')+'</span><span>'+totalSeats+'</span></div>'
      + '</div>'
      + '<div class="prob-row">'
      + '<div><div class="prob-label">'+t('Dem majority','Majorité Dem')+'</div><div class="prob-value c-dem">'+Math.round(pDem*100)+'%</div></div>'
      + '<div><div class="prob-label">'+t('GOP majority','Majorité GOP')+'</div><div class="prob-value c-gop">'+Math.round((1-pDem)*100)+'%</div></div>'
      + '</div></div>';
  }
  grid.innerHTML = chamberCard(t('House','Chambre'), hSeats, 435-hSeats, 435, hMaj, pHouse) +
                   chamberCard(t('Senate','Sénat'),  sSeats, 100-sSeats, 100, sMaj, pSenate);
}

// ── Tooltip (global, one listener) ──────────────────────────────────────────
function setupTooltip() {
  ...
  document.addEventListener('mousemove', e => {
    ...
    if (el) {
      ...
      if (ttLabel) ttLabel.textContent = parts[0]||'';
      if (ttBody)  ttBody.textContent  = parts[1]||'';
...
// ── Islands: lazy init via IntersectionObserver ──────────────────────────────
function lazyInit(elementId, callback) {
  const el = document.getElementById(elementId);
  if (!el) return;
  const obs = new IntersectionObserver(entries => {
    if (entries[0].isIntersecting) {
      obs.disconnect();
      getLdiData().then(data => { if (data) callback(data); }).catch(console.error);
    }
...
// ── Boot ─────────────────────────────────────────────────────────────────────
// Above-fold sections: init immediately
getLdiData().then(data => {
  if (!data) {
    const sub = document.querySelector('.hero-sub');
    if (sub) sub.textContent += t(' (data unavailable — check back later)', ' (données indisponibles)');
    return;
  }
...
// Below-fold sections: defer until viewport entry
lazyInit('comp-grid',    data => renderComponentGrid(data.components || []));
lazyInit('main-chart',   data => renderChart(data.history || []));
lazyInit('history-grid', data => {
  renderChart(data.history || []);          // ensures _chartData is populated
  renderPresidentsGrid(data.historical_presidents || [], data.ldi.score);
});
lazyInit('midterms-grid', data => renderMidterms(data.midterms || {}));

// Tooltip is global — init after DOM ready
setupTooltip();
