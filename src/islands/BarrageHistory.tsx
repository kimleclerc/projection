import type { BarrageHistoryPoint, BarrageLocale } from '../data/barrage';

/* BarrageHistory — daily-readings sparkline for the Barrage Index (key: bfi).
 * Same chart geometry as GooseHistory, kept separate so the shared islands
 * never grow instrument-specific switches. */
...
export default function BarrageHistory({ history, currentScore, locale }: Props) {
  const t = copy[locale] ?? copy.en;
  const pts = history.filter((h) => typeof h.bfi === 'number') as Array<Required<Pick<BarrageHistoryPoint, 'date' | 'bfi'>> & BarrageHistoryPoint>;
    ...
    return (
      <div class="note-card" style="max-width:560px">
        ...
        <p style="font-family:var(--mono);color:var(--ink-3);margin-top:.5rem">
          {t.latest}: {locale === 'en' ? currentScore.toFixed(1) : currentScore.toFixed(1).replace('.', ',')}/100 · {pts.length} {t.points}
        </p>
      </div>
    );
  }
  ...
  const ys = pts.map((p) => padT + (1 - (p.bfi as number) / 100) * (H - padT - padB));
  ...
  return (
    <div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style="width:100%;height:auto;overflow:visible" role="img" aria-label={t.title}>
        <defs>
          <linearGradient id="barrageArea" x1="0" y1="0" x2="0" y2="1">
        ...
        <path d={area} fill="url(#barrageArea)" />
